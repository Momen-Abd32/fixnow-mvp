...
export async function register(name: string, email: string, password: string): Promise<{ sessionToken: string; user: any }> {
  const result = await apiCall<{ token: string; user: any }>("/api/auth/register", {
    method: "POST",
